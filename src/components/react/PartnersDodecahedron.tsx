...
const PARTNERS: { name: string; logo: string }[] = [
  { name: 'KAWNEER', logo: '/logos/kawneer.svg' },
  { name: 'YKK AP', logo: '/logos/ykk-ap.png' },
  { name: 'OLDCASTLE', logo: '/logos/oldcastle.png' },
  { name: 'VIRACON', logo: '/logos/viracon.svg' },
  { name: 'AGC GLASS', logo: '/logos/agc.png' },
  { name: 'TUBELITE', logo: '/logos/tubelite.png' },
  { name: 'ATLAS', logo: '/logos/atlas.png' },
  { name: 'EFCO', logo: '/logos/efco.svg' },
  { name: 'STANLEY', logo: '/logos/stanley.svg' },
  { name: 'GUARDIAN', logo: '/logos/guardian.png' },
  { name: 'PPG', logo: '/logos/ppg.svg' },
  { name: 'ARCADIA', logo: '/logos/arcadia.png' },
];
...
function extractFaces(radius: number): FaceData[] {
  ...
  for (let f = 0; f < 12; f++) {
    ...
    const tangent = unique[0].clone().sub(center).normalize();
    ...
    faces.push({ geometry: faceGeo, center, normal });
  }
...
function Scene({ onHover }: { onHover: (idx: number | null) => void }) {
  ...
  useEffect(() => {
    materials.forEach((mat, i) => {
      mat.opacity = hoveredFace === i ? 1 : 0.88;
      mat.emissive = hoveredFace === i ? new THREE.Color(0x4A90D9) : new THREE.Color(0x000000);
      mat.emissiveIntensity = hoveredFace === i ? 0.15 : 0;
    });
  }, [hoveredFace, materials]);
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      vel.current = {
        x: (e.clientY - dragStart.current.y) * 0.003,
        y: (e.clientX - dragStart.current.x) * 0.003,
      };
      dragStart.current = { x: e.clientX, y: e.clientY };
    };
    const up = () => {
  ...
  useFrame((_, dt) => {
    ...
    if (autoRotate.current) {
      ...
      groupRef.current.rotation.x += (Math.sin(t) * 0.12 - groupRef.current.rotation.x) * 0.015;
    } else {
...
/* ═══════════════════════════════════════════════════════
   HTML overlay — hover info panel
   ═══════════════════════════════════════════════════════ */

function InfoPanel({ partner }: { partner: (typeof PARTNERS)[number] | null }) {
  return (
    <div
...
export default function PartnersDodecahedron() {
  ...
  return (
    <div className="relative w-full" style={{ height: '650px' }}>
      ...
      <InfoPanel partner={hovered !== null ? PARTNERS[hovered] : null} />
